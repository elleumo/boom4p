...
SidePanel::SidePanel(const lif::LevelManager& lm, unsigned firstIdx, unsigned count, bool alignRight)
	: lm(lm)
{
	// Load background
...
for (unsigned i = 0; i < playerHeadsSprite.size(); ++i) {
    unsigned playerIndex = firstIdx + i; // Mostra le teste corrette
    playerHeadsSprite[i].setTexture(*playerHeadsTexture);
    playerHeadsSprite[i].setTextureRect(sf::IntRect(
        PLAYER_HEAD_WIDTH * playerIndex, 0,
...
playerHeadsSprite[1].setPosition(PLAYER1_HEAD_X, PLAYER2_HEAD_Y);
	...
	for (unsigned i = 0; i < playerHeadsSprite.size(); ++i) {
    unsigned playerIndex = firstIdx + i; // P1/P2 se firstIdx=0, P3/P4 se firstIdx=2
    playerHeadsSprite[i].setTexture(*playerHeadsTexture);
    playerHeadsSprite[i].setTextureRect(sf::IntRect(
        PLAYER_HEAD_WIDTH * playerIndex, 0,
        PLAYER_HEAD_WIDTH, PLAYER_HEAD_HEIGHT
		...
		const lif::Player& player) const
{
	...
	auto row = static_cast<unsigned>((player.getInfo().id - 1 - firstIdx) & 1u);
	sf::Vector2f pos(HEALTH_SYM_POS_X, row == 0 ? HEALTH_SYM_POS_Y_1 : HEALTH_SYM_POS_Y_2);
...
void SidePanel::_drawExtraLetters(sf::RenderTarget& window, sf::RenderStates states, const lif::Player& player) const {
	...
	auto row2 = static_cast<unsigned>((player.getInfo().id - 1 - firstIdx) & 1u);
	sf::Vector2f pos(EXTRA_LETTERS_POS_X, row2 == 0 ? EXTRA_LETTERS_POS_Y_1 : EXTRA_LETTERS_POS_Y_2);
...
void SidePanel::draw(sf::RenderTarget& window, sf::RenderStates states) const {
	const float originX = alignRight ? lif::SIDE_PANEL_WIDTH + lif::GAME_WIDTH : 0.f;
	states.transform.translate(originX, 0.f);
	...
	for (unsigned i = 0; i < playerHeadsSprite.size(); ++i) {
		...
		std::stringstream ss;
		const auto player = lm.getPlayer(firstIdx + i + 1);
		if (player == nullptr)
			ss << "X0";
		else {
		...
		ss.str("");
		ss << std::setfill('0') << std::setw(6) << lm.getScore(firstIdx + i + 1);
		//scoreText.setCharacterSize(16);
...
void SidePanel::update() {
	...
	for (unsigned i = 0; i < count; ++i) {
		const unsigned pid = firstIdx + i;
		const auto player = lm.getPlayer(pid + 1);
