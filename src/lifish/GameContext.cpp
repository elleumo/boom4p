...
GameContext::GameContext(sf::Window& window, const std::string& levelsetName, short startLv)
	...
	, window(window)
	, lm()
	, leftPanel(lm, 0, 2, false)
	, rightPanel(lm, 2, 2, true)
	, wlHandler(lm, leftPanel)
...
void GameContext::update() {
	...
	if (!lm.isPaused()) {
		lm.update();
		leftPanel.update();
	if (lif::options.nPlayers > 2) rightPanel.update();
		gameTimer.update();
	}
...
void GameContext::draw(sf::RenderTarget& window, sf::RenderStates states) const {
	...
	window.draw(leftPanel, states);
	window.draw(rightPanel, states);
